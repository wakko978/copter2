class Therian < General
  ...
  def monster_attack(recruit)
    ...
    case recruit.level
    when 1..4
      monster_attack += recruit.level * 5 + 5 + mod1 + mod2
    when 5..50
      monster_attack += (recruit.level - 5) * 30 + 5 + 50 + mod1 + mod2
    end
  ...
  def attack_with_mods(profile,recruit)
    ...
    attack += 1 if profile.inventory_exists?('weapons','Silverfist Hammer')
  ...
  def defense_with_mods(profile,recruit)
    ...
    defense += 1 if profile.inventory_exists?('items','Silverfist Plate')
